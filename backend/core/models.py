...
class Usuario(AbstractUser):
    ...
    last_password_change = models.DateTimeField(null=True, blank=True)  # Fecha del último cambio de contraseña
    password_change_date = models.DateTimeField(null=True, blank=True)  # Fecha de la próxima renovación de contraseña
    email = models.TextField()  # Cambiado de CharField a TextField para manejar la longitud en la encriptación
    ...
    is_for_login = models.BooleanField(default=False)  # Indica si el código es para login o cambio de contraseña
    failed_attempts = models.IntegerField(default=0)
    is_locked = models.BooleanField(default=False)
# Campos para preguntas de seguridad
    favorite_color = models.TextField(null=True, blank=True)  # Cambiado a TextField
    ...
    def set_password_change_date(self):
        ...
        self.password_change_date = self.last_password_change + timedelta(days=90)
    ...
    def enable_new_password(self, new_password):
        ...
        self.save(update_fields=['password', 'verify_code', 'last_password_change', 'password_change_date'])
...
class AuditoriaUsuario(models.Model):
    usuario = models.ForeignKey(Usuario, on_delete=models.CASCADE, verbose_name="Usuario")  # Relación con el ID específico del usuario
    accion = models.CharField(max_length=255, verbose_name="Acción")  # Tipo de acción realizada
    fecha_hora = models.DateTimeField(auto_now_add=True, verbose_name="Fecha y Hora")
    detalles = models.TextField(null=True, blank=True, verbose_name="Detalles")  # Detalles adicionales de la acción (opcional)

    def __str__(self):
        return f"Auditoría: {self.usuario.username} - {self.accion} en {self.fecha_hora}"
...
class EstudioPDF(models.Model):
    ...
    ruta = models.CharField(max_length=500)  # Ruta del archivo PDF en el servidor
    estado = models.CharField(max_length=50)  # Estado del estudio (activo, inactivo, etc.)
    fecha_subida = models.DateField(auto_now_add=True)  # Fecha de subida del archivo
...
class ArchivoResultados(models.Model):
    estudio = models.ForeignKey(EstudioPDF, on_delete=models.CASCADE)  # Relación con la tabla de estudios PDF
    tipo_analisis = models.CharField(max_length=100)
    ruta = models.CharField(max_length=255, blank=True, null=True)  # Ruta del archivo CSV/Excel
    contenido = models.JSONField(blank=True, null=True)  # Almacena el contenido procesado del archivo
...
class VisualizacionDatos(models.Model):
    resultado = models.ForeignKey(ArchivoResultados, on_delete=models.CASCADE)  # Relación con Resultados
...
class DatosEstandarizados(models.Model):
    archivo = models.ForeignKey(ArchivoResultados, on_delete=models.CASCADE)  # Relación con ArchivoResultados
    encabezados = models.JSONField()  # Encabezados del archivo procesado (formato flexible)
    contenido = models.JSONField()  # Contenido del archivo estandarizado en formato JSON
